...
**Corollary 4.2 (Observation Stabilizes).** *Applying an observer to its own output returns the same output.* ∎

**Theorem 4.3 (R(R) = R Is Both Definition and Theorem).** *As a definition: an idempotent is a morphism f with f ∘ f = f. As a theorem: given only Dist (forced by existence), q ∘ q = q is a consequence — not a postulate.* ∎
